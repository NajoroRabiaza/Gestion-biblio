<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Gestion des Auteurs</h2>
    </x-slot>
    ...
    <style>
        ...
        /* toast — caché complètement hors écran */
        .toast {
            position: fixed;
            top: 24px;
            left: 24px;
            ...
            align-items: center;
            gap: 10px;
            /* calc(-100% - 24px) = sort complètement à gauche + marge */
        ...
        .toast.show { transform: translateX(0);
            opacity: 1;
            visibility: visible; }
        .toast.hide { transform: translateX(calc(-100% - 24px)); transition: transform 0.3s ease-in, opacity 0.3s ease-in;
            opacity: 0;
            visibility: hidden; }
    ...
    <div class="page-bg py-10 px-4">
        <div style="max-width: 860px; margin: 0 auto;">
            ...
            <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 28px;">
                <div>
                    <h1 class="section-title">Auteurs</h1>
            ...
            @if($authors->isEmpty())
                <div class="empty-state"><p>Aucun auteur enregistré pour le moment.</p></div>
            @else
                <div id="authors-list">
                @foreach($authors as $author)
                    <div class="row-card" id="card-{{ $author->id }}">
                        <div>
                            <div class="row-name">{{ $author->name }}</div>
                            <div class="row-sub">
                                {{ $author->nationality ?? 'Nationalité inconnue' }}
                                @if($author->birth_date)
                                    &nbsp;—&nbsp; Né(e) le {{ \Carbon\Carbon::parse($author->birth_date)->format('d/m/Y') }}
                                @endif
                            </div>
                        </div>
                        <div class="right-block">
                            <span class="badge-count">{{ $author->books_count }} livre(s)</span>
                            <button
                                class="btn-icon-delete"
                                data-id="{{ $author->id }}"
                                data-url="{{ route('admin.authors.destroy', $author->id) }}"
                                onclick="supprimerAuteur(this)"
                                title="Supprimer"
                            >
    ...
    <script>
        ...
        function supprimerAuteur(btn) {
            demanderConfirmation('Supprimer cet auteur ?', 'Supprimer', function() {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        setTimeout(() => {
                            ...
                            document.getElementById('subtitle').textContent = remaining + ' auteur(s) enregistré(s)';
                        }, 380);
                    } else {
